    ...
    var methods = {
        init: function(opts){
            ...
            return this.each(function(){
                ...
                // construct internal elements   
                var cliOutput = $('<div>').addClass('cli-output'); 
                $this.append(cliOutput);
                
                var pinPanel = $('<div>').addClass('cli-pinPanel')
                    .append($('<div>').addClass('cli-pinPanel-close'))
                    .append($('<div>').addClass('cli-pinPanel-title').text('Pin Panel'));
                
                $this.append(pinPanel);

                var cliInput = $('<input>').addClass('cli-input').attr('type', 'text');    
                var cliPrompt = $('<div>').addClass('cli-promptText');
                var inputContainer = $('<div>').addClass('cli-input-container')
                    .append(cliPrompt)
                    .append($('<div>').addClass('cli-input-wrapper').append(cliInput))
                    .append($('<div>').addClass('cli-input-container-clear'));
                
                $this.append(inputContainer);
                
                opts.promptControl = cliPrompt;
                var options = $.extend(true, {}, $.fn.cli.defaults, opts);

                // the plugin hasn't been initialized yet
                var sessionId = (queryObject.get('sid') || options.sid || 'auto'+(defaultSessionIdCounter++)) + '_',
                    envStorageKey = sessionId + 'env',
                    pluginsStorageKey = sessionId + 'commands',
                    historyStorageKey = sessionId + "history",
                    resultsContainer = $("<div>").appendTo(cliOutput),
                ...
                function getCommand() {
                    return cliInput.val().trim();
                }

                function setCommand(command) {
                    cliInput.focus().val('');
                    setTimeout(function(){
                        cliInput.val(command);
                    }, 1);
                }
                ...
                function executeCommand(command, args) {
                  ...
                  function execCommandInternal() {
                    ...
                    function appendExecutionResult(result) {
                        ...
                        if (isPin) {
                            panel.addItemToPinPanel(resultContainer);
                            pinPanel.show();
                        }
                        else
                            if (!isLive || liveCount === 0) scrollDown();
                    }
                  }
                }
                ...
                function addSystemParams() {
                  ...
                  systemParams.push({
                    name: 'pin',
                    description: 'pins the result on the panel',
                    type: 'boolean'
                  });
                ...
                function addDefaultCommands() {
                    ...
                    var pinPanelCommand = {
                        name: 'panel',
                        description: 'Manages the panel panel. Using with no params will toggle the panel visibility.',
                        usage: 'panel [--off] [--on] [--pin]',
                        example: 'panel',
                        params: [
                            {
                                name: 'off',
                                description: 'turn panel visibility off',
                                type: 'boolean'
                            },
                            {
                                name: 'on',
                                description: 'turn panel visibility on',
                                type: 'boolean'
                            },
                            {
                                name: 'pin',
                    ...
                    var tabCommand = {
                        ...
                        exec: function (args, context) {
                            ...
                            url = args.name ? url.addQuery('sid', args.name) : url.removeQuery('sid');
                            window.open(url);
                        }
                    }
